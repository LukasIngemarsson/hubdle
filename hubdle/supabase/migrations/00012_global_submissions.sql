...
-- 1. Drop RLS policy that depends on group_id before dropping the column
drop policy "Group members can view submissions" on public.submissions;

-- 2. Deduplicate: keep the earliest submission per (user_id, game_id, game_date)
...
-- 3. Drop the old unique constraint and group_id column
...
-- 4. Add new unique constraint: one score per user per game per day
alter table public.submissions
  add constraint submissions_user_id_game_id_game_date_key
  unique (user_id, game_id, game_date);

-- 5. New RLS: any authenticated user can view submissions
