	...
	<div class="mt-6 grid gap-6">
		<div class="grid grid-cols-3 gap-4">
			<div class="flex items-center justify-center gap-3 rounded-lg bg-base-200 p-4">
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-8 w-8 shrink-0 text-orange-500"><path fill-rule="evenodd" d="M12.963 2.286a.75.75 0 0 0-1.071-.136 9.742 9.742 0 0 0-3.539 6.176 7.547 7.547 0 0 1-1.705-1.715.75.75 0 0 0-1.152-.082A9 9 0 1 0 15.68 4.534a7.46 7.46 0 0 1-2.717-2.248ZM15.75 14.25a3.75 3.75 0 1 1-7.313-1.172c.628.465 1.35.81 2.133 1a5.99 5.99 0 0 1 1.925-3.546 3.75 3.75 0 0 1 3.255 3.718Z" clip-rule="evenodd" /></svg>
				<div>
					<p class="text-2xl font-bold leading-tight">{data.stats.streak}</p>
					<p class="text-sm opacity-60">Day streak</p>
				</div>
			</div>
			<div class="flex items-center justify-center gap-3 rounded-lg bg-base-200 p-4">
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-8 w-8 shrink-0 opacity-70"><path d="M8.25 6.75a3.75 3.75 0 1 1 7.5 0 3.75 3.75 0 0 1-7.5 0ZM15.75 9.75a3 3 0 1 1 6 0 3 3 0 0 1-6 0ZM2.25 9.75a3 3 0 1 1 6 0 3 3 0 0 1-6 0ZM6.31 15.117A6.745 6.745 0 0 1 12 12a6.745 6.745 0 0 1 6.709 7.498.75.75 0 0 1-.372.568A12.696 12.696 0 0 1 12 21.75c-2.305 0-4.47-.612-6.337-1.684a.75.75 0 0 1-.372-.568 6.787 6.787 0 0 1 1.019-4.38Z" /><path d="M5.082 14.254a8.287 8.287 0 0 0-1.308 5.135 9.687 9.687 0 0 1-1.764-.44l-.115-.04a.563.563 0 0 1-.373-.487l-.01-.121ZM19.573 19.389a8.287 8.287 0 0 0-1.308-5.135 3.75 3.75 0 0 1 3.57 4.047l-.01.121a.563.563 0 0 1-.373.486l-.115.04c-.567.2-1.156.349-1.764.441Z" /></svg>
				<div>
					<p class="text-2xl font-bold leading-tight">{data.stats.friendCount}</p>
					<p class="text-sm opacity-60"><a href="/users/{data.profile.username}/friends" class="underline">{data.stats.friendCount === 1 ? 'Friend' : 'Friends'}</a></p>
				</div>
			</div>
			<div class="flex items-center justify-center gap-3 rounded-lg bg-base-200 p-4">
				{#if data.stats.favoriteGame && GAME_ICONS[data.stats.favoriteGame.id]}
					<img src={GAME_ICONS[data.stats.favoriteGame.id]} alt="" class="h-8 w-8 shrink-0 rounded" />
				{/if}
				<div>
					<p class="text-2xl font-bold leading-tight">{data.stats.favoriteGame?.name ?? '—'}</p>
